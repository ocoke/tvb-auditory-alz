#!/usr/bin/env python3
"""Run the canonical extended late-window TVB379 experiment."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the immutable LateWindowFollowup 379-region semantic-"
            "versus-episodic musical-memory proxy experiment. The default "
            "is the locked 1,242-call final workload: 762 unchanged primary "
            "calls plus 480 prolonged follow-up calls, with 300 lossless "
            "trace shards."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "verify the exact notebook hash and contract, compile all code "
            "cells, print the locked workloads, and exit without running TVB"
        ),
    )
    parser.add_argument(
        "--resume",
        ...
        default=None,
        metavar="RUN_DIR",
        help="resume an incomplete run of this exact extended notebook",
    )
    parser.add_argument(
        "--status",
        type=Path,
        default=None,
        metavar="RUN_DIR",
        help="print the saved status of an extended notebook run and exit",
    )
    return parser
...
def _configure_environment(
    ...
    *,
    ...
    if args.resume is not None:
        os.environ["RISE_RUN_ID"] = f"resume_bootstrap_{os.getpid()}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(format_extended_validation_summary(validation), flush=True)
    if args.check:
        print(
            "Static LateWindowFollowup check passed; no TVB calls ran."
        )
        return 0
